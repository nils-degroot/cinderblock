...
//   resource! macro → ash_sqlx extension codegen → SqliteDataLayer CRUD
//
// Each test creates a fresh in-memory SQLite database, applies the schema,
// registers the data layer on a Context, and exercises the ash_core CRUD
...
use ash_sqlx::sqlite::SqliteDataLayer;
...
async fn create_and_read_back_via_list() {
    ...
    let created = ash_core::create::<Task, Add>(
        AddInput {
    ...
    let tasks = ash_core::list::<Task>(&ctx).await.expect("list tasks");
...
async fn read_by_primary_key() {
    ...
    let fetched = ash_core::data_layer::DataLayer::<Task>::read(dl, &created.task_id)
        .await
...
async fn update_modifies_row() {
    ...
    let updated = ash_core::update::<Task, Complete>(&created.task_id, CompleteInput {}, &ctx)
        .await
        .expect("update task");
...
async fn destroy_deletes_and_returns_resource() {
    ...
    let destroyed =
        ash_core::destroy::<Task, Remove>(&created.task_id, &ctx)
            .await
            .expect("destroy task");
...
async fn destroy_nonexistent_returns_error() {
    ...
    let result = ash_core::destroy::<Task, Remove>(&fake_id, &ctx).await;
...
async fn create_multiple_then_destroy_one() {
    ...
    let task_a = ash_core::create::<Task, Add>(
        AddInput {
    ...
    let task_b = ash_core::create::<Task, Add>(
        AddInput {
    ...
    ash_core::destroy::<Task, Remove>(&task_b.task_id, &ctx)
        .await
        .expect("destroy task B");

    let remaining = ash_core::list::<Task>(&ctx).await.expect("list tasks");
